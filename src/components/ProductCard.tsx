...
const ProductCard = ({ product, compact = false }: ProductCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">{product.category}</p>
        <h3 className="text-sm font-semibold text-foreground leading-tight mt-0.5 line-clamp-1">
          {product.name}
        </h3>
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-sm font-bold text-foreground">{product.price.toLocaleString()} ETB</span>
          {product.oldPrice && (
            <span className="text-xs text-muted-foreground line-through">{product.oldPrice.toLocaleString()} ETB</span>
          )}
        </div>
      </div>
    </div>
