...
const Brands = () => {
  ...
  // Build backend-ready filter object
  const productFilters = useMemo<ProductFilters>(() => {
    ...
    // collections are client-side combined filters for now
    ...
    return f;
  }, [selectedBrand, search, gender, filters]);

  const { products: filtered } = useProducts(productFilters);

  // Additional client-side filtering for multiple categories (will be backend later)
  ...
  return (
    <div className="pb-20">
      ...
      <BrandChips selected={selectedBrand} onSelect={setSelectedBrand} showAll />
